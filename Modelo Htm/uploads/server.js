...
const bcrypt = require('bcrypt'); // 👈 AGREGADO: Requerido para contraseñas
...
app.get('/api/admin/dashboard', async (req, res) => {
    try {
        ...
        const [usuarios] = await db.query('SELECT COUNT(*) as usuarios_total FROM usuarios');
        const [productos] = await db.query('SELECT COUNT(*) as productos_total FROM productos');
        ...
        const [tiposUsuario] = await db.query('SELECT tipo as nombre, COUNT(*) as cantidad FROM usuarios GROUP BY tipo');
...
app.post('/api/registro/vendedor', async (req, res) => {
    ...
    try {
        ...
        conexion.release(); // 👈 IMPORTANTE: Asegura liberar la conexión del pool
    }
});

// ── PRODUCTOS (Ruta completada) ───────────────────────────────────
app.get('/api/productos', async (req, res) => {
    try {
        const [productos] = await db.query(`
            SELECT p.id_producto AS id, p.nombre, p.descripcion, p.precio, p.stock, p.id_categoria, c.nombre AS categoria, COALESCE(SUM(dv.cantidad), 0) AS ventas
            FROM Producto p
